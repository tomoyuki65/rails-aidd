...
### app/container（DIコンテナ層）

- ユースケースの実行単位と依存関係（DI）を組み立てる層
  - ビジネスロジックは持たない
  - ユースケースの生成と依存注入のみを担当する
  - コントローラーから呼ばれる実行入口を提供する
- domainsのユースケースはcontainerで組み立てる
- 必要に応じてfeaturesのユースケースを組み立てることも可能とする

#### 基本原則

- container配下では「業務のまとまり」でディレクトリやファイルを分割する
  - 例：
    - user（ユーザー管理）
    - order（注文管理）
    - signup（登録フロー）
    - checkout（購入フロー）
- 実装場所（domains / features）は単独の判断基準にしない
  - container配下で分割する際は、「どこに実装されているか（domains / features）」だけで構造を決めない
- container配下の単位はユースケースの文脈である
  - ディレクトリやファイルは、「何の業務か説明できる単位」で切ること
    - モデル単位でない
    - レイヤー単位でない
    - 技術単位でない
- domainsとfeaturesのcontainerは混在させない
  - 必要に応じてfeaturesのユースケースを組み立ててもよいが、domainsで切ったディレクトリやファイルに混在させないこと
- container配下のディレクトリやファイルは技術構造で切らない
  - 以下は禁止：
    - domains / features で分割
    - domains/[domain_name]/domain/use_case単位で分割
    - features/services単位で分割
- 将来のユースケース増加を前提とする
  - ユースケースは増える前提
  - DI構成も増える前提
  - リファクタリング前提で設計する
...
- domainsのユースケースはcontainerで組み立てたものをcontrollerから呼び出す。
- 必要に応じてcontainerで組み立てたfeaturesのユースケースをcontrollerから呼び出す。
- 内部実装やモデルは直接呼ばない。

### 例

- containerで組み立てたユースケース
